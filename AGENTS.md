...
## Design to code (designers)

When implementing visual changes or full pages, follow **[DESIGN-TO-CODE.md](./DESIGN-TO-CODE.md)** and **[studiongsoft/design-lead](https://github.com/studiongsoft/design-lead)**:

- Site is **RTL** — use `AppThemeProvider` from `@studiongsoft/design-lead`.
- **Pages compose existing components only**; change components in `packages/mui-material/src/` and document in `design-decisions/`.
- **Light + dark** required for every component change.
- Design conflicts need **Design Lead** approval (`needs-design-approval`, `.github/CODEOWNERS`).

Cursor rules: `.cursor/rules/design-*.mdc`. PR template: `.github/pull_request_template/design_system.md`.

**Figma → code:** Cursor skill `figma-design-implementation` (`.cursor/skills/figma-design-implementation/`). Stops when Figma ≠ component code; documents each component in `design-decisions/components/<Name>.md`.

**Design team guides:** [docs/design-team/README.md](./docs/design-team/README.md) — contract, page build guide, checklists.

**SPA app:** `apps/keren-or/` — run with `pnpm -F keren-or dev`.
